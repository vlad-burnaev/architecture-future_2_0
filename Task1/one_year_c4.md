...
flowchart TB
  subgraph Clinical["Домен клиники"]
    UI["Клиентский интерфейс оператора"]
...
- Внешние партнёры связаны через шину и контролируемые API доменные потоки партнёров попадают в lakehouse платформы без расширения логики в центральном DWH.
- Внутренние сервисы домена клиники показаны как источник истины. Интерфейс оператора в первую очередь работает с ними. Медицинские данные остаются в регламентированных контурах.
